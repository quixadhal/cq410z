 ...
 /* 0 is for full report 'a' and 1 is for group report 'g' */
{
  ...
  int             isgod = FALSE;
  int             count,
                  count2;	/* number of armies on current screen */
  short           armynum = 0;
  ...
  while (done == FALSE) {
...
#ifdef OGOD
      if (isgod == TRUE)
	mvaddstr(ypos++, 0, "GOD OPTIONS: 6) LOCATION 7) SOLDIERS 8) MOVE 9) UNITTYPE 0) STATUS ");
      clrtoeol();
#endif	/* OGOD */
	...
	if (P_ATYPE < MINLEADER)
	  ...
	  mvprintw(ypos++, 0, "Disband your %s %d?", *(unittype + (P_ATYPE % UTYPE)), armynum);
	refresh();
	if (getch() != 'y')
	  break;
...
#ifdef OGOD
      if (isgod == TRUE)
	mvaddstr(ypos++, 0, "GOD OPTIONS:  4) ADJUST SHIPS, 5) LOCATION, 6) CREW 7) MOVE");
      ypos++;
      clrtoeol();
#endif	/* OGOD */
