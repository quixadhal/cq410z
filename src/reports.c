 ...
 * Fixed army and group reports to use relative coordinates.
 *
...
/* 0 is for full report 'a' and 1 is for group report 'g' */
{
  ...
  while (done == FALSE) {
    ...
    while ((armynum < MAXARM) && (count < MAXINSCR)) {
      if (P_ASOLD > 0 && (!repnum || (P_AXLOC == XREAL && P_AYLOC == YREAL))) {
        ...
        if (isgod) {
          ...
          mvprintw(ypos + 2, xpos, "%d", rel_x((int) P_AXLOC));
          mvprintw(ypos + 3, xpos, "%d", rel_y((int) P_AYLOC));
        }
        ...
        if (P_ATYPE < MINMONSTER)
          mvprintw(ypos + 7, xpos, "$%ld",
                   P_ASOLD * *(unitmaint + (P_ATYPE % UTYPE)));
        else
          mvprintw(ypos + 7, xpos, "$%ld",
                   5L * *(unitmaint + (P_ATYPE % UTYPE)));
        if (P_ATYPE < MINMONSTER)
          mvprintw(ypos + 8, xpos, "$%ld",
                   P_ASOLD * *(u_encost + (P_ATYPE % UTYPE)));
        else
          mvprintw(ypos + 8, xpos, "%d pts",
                   (int) * (u_encost + (P_ATYPE % UTYPE)));
        count++;
    ...
    mvprintw(0, (COLS / 2) - 15 - strlen(curntn->name) / 2,
             "ARMY STATS SUMMARY FOR %s", curntn->name);

    ypos += 10;
    mvaddstr(ypos++, (COLS / 2) - 14, "HIT SPACE KEY IF DONE");
    mvaddstr(ypos++, (COLS / 2) - 25,
             "HIT RETURN TO CHANGE ANY ARMY STATISTICS");
    ...
    if ((inpkey = getch()) == ' ')
...
#ifdef OGOD
      if (isgod == TRUE)
        mvaddstr(ypos++, 0, "GOD OPTIONS: 6) LOCATION 7) SOLDIERS 8) MOVE 9) "
                            "UNITTYPE 0) STATUS ");
      clrtoeol();
#endif /* OGOD */
      ...
      switch (getch()) {
        ...
        case '2': /* disband army */
          clear_bottom(0);
          if (P_ATYPE < MINLEADER)
            mvprintw(ypos++, 0, "Disband your %s army %d?",
                     *(unittype + (P_ATYPE % UTYPE)), armynum);
          else
            mvprintw(ypos++, 0, "Disband your %s %d?",
                     *(unittype + (P_ATYPE % UTYPE)), armynum);
          refresh();
          if (getch() != 'y')
            break;
          if (sct[P_AXLOC][P_AYLOC].owner != country && P_ATYPE != A_SPY) {
            ...
            mvprintw(LINES - 4, 0,
                     "Your spy demands %ld talons to remain quiet",
            ...
            mvprintw(LINES - 4, 0,
                     "Your mercenaries demand %ld talons to disband",
            ...
            if (P_ATYPE < MINLEADER) {
              ...
              /* 15% become mercs */
              attset = curntn->aplus + *(unitattack + (P_ATYPE % UTYPE));
              defset = curntn->dplus + *(unitdefend + (P_ATYPE % UTYPE));
              P_ASOLD -= bemerc;
        ...
        case '9':
          if (isgod == TRUE) {
            mvaddstr(
                ypos++, 0,
                "Change unit type; Normal 0-26, Leader 27-44, Monster 45-59");
            mvaddstr(ypos++, 0, "[MajorHackEH?] New Type is? ");
            refresh();
            men = get_number();

            if (men < 0 || men > MAXMONSTER)
              break;
            if (men > 44)
              men += TWOUTYPE;
            else if (men > NOUNITTYPES)
              men += UTYPE;
        ...
        case '0':
          if (isgod == TRUE) {
            mvprintw(ypos++, 0, "1) Mar 2) Sct 3) Garr 4) Trade 5) Mil 6) Fly "
                                "7) Def 8) MDef 9) Att 10) MAtt");
            mvprintw(ypos++, 0, "11) Genrl 12) Sort 13) Sieg 14) Sgd 15) Onb "
                                "16) Rule 17+) Group (leader-=17)");
      ...
      while (armynum < MAXARM &&
             (P_ASOLD == 0 ||
              !(!repnum || (P_AXLOC == XREAL && P_AYLOC == YREAL))))
        armynum++;
      if (armynum == MAXARM)
        count2 = 0;
      else
...
void budget(void) {
  ...
  mvprintw(2, COLS - 50, "%8ld people in gold mines.%8ld", spread.ingold,
           spread.revjewels);
  mvprintw(3, COLS - 50, "%8ld people in mines......%8ld", spread.inmetal,
           spread.revmetal);
  mvprintw(4, COLS - 50, "%8ld people in farms......%8ld", spread.infarm,
           spread.revfood);
  mvprintw(5, COLS - 50, "%8ld people in cities.....%8ld", spread.incap,
           spread.revcap);
  mvprintw(6, COLS - 50, "%8ld people in towns......%8ld", spread.incity,
           spread.revcity);
  mvprintw(7, COLS - 50, "%8ld people elsewhere.....%8ld", spread.inothr,
           spread.revothr);
  standout();
  mvprintw(8, COLS - 50, "%8ld people INCOME........%8ld", spread.civilians,
           spread.gold - curntn->tgold);
  standend();
  mvprintw(10, COLS - 50, "%8ld troops...............%8ld", numsold, costsold);
  mvprintw(11, COLS - 50, "%8ld monsters.............%8ld", nummonst,
           costmonst);

  mvprintw(12, COLS - 50, "%8ld ship holds @ %4d....%8ld", numship, SHIPMAINT,
           numship * SHIPMAINT);
  mvprintw(13, COLS - 50, "other expenses this turn......%8ld",
           startgold - curntn->tgold);
  standout();
  money =
      costmonst + costsold + (numship * SHIPMAINT) + startgold - curntn->tgold;
  ...
  mvprintw(17, COLS - 50, "LOSS DUE TO INFLATION.........%8ld",
           (startgold + money - chty) - infmoney);
  standout();
...
void produce(void) {
  ...
  mvprintw(10, 0, "%8ld people in farms..%8ld tons", spread.infarm,
           spread.food - curntn->tfood);
  fprintf(fp, "LINE %d FILE %s\n", __LINE__, __FILE__);
  mvprintw(12, 0, "%8ld people @ %3.1f eat.%8.0ld tons", spread.civilians,
           P_EATRATE, (long)(P_EATRATE * (float) spread.civilians));
  fprintf(fp, "LINE %d FILE %s\n", __LINE__, __FILE__);
  mvprintw(13, 0, "%8ld soldiers eat.....%8.0ld tons", military,
           (long)(military * 2 * P_EATRATE));
  military += military + spread.civilians; /* military is amount eaten */
  fprintf(fp, "LINE %d FILE %s\n", __LINE__, __FILE__);
  standout();
  mvprintw(15, 0, "ESTIMATE NET FOOD.........%8.0f tons",
           spread.food - curntn->tfood - P_EATRATE * military);
  mvprintw(16, 0, "ESTIMATE FOOD SUPPLY......%8.0f tons",
           spread.food - P_EATRATE * military);
  standend();
  fprintf(fp, "LINE %d FILE %s\n", __LINE__, __FILE__);

  mvaddstr(7, COLS / 2 + 1, "OTHER PRODUCTION");
  mvprintw(9, COLS / 2 + 1, "jewels owned is...............%8ld",
           curntn->jewels);
  mvprintw(10, COLS / 2 + 1, "%8ld people in goldmines..%8ld", spread.ingold,
           spread.jewels - curntn->jewels);
  mvprintw(11, COLS / 2 + 1, "%8ld monsters.............%8ld", nummonst,
           -costmonst);
  standout();
  mvprintw(12, COLS / 2 + 1, "ESTIMATE JEWEL SUPPLY.........%8ld",
           spread.jewels - costmonst);
  standend();

  mvprintw(14, COLS / 2 + 1, "metal ore owned is............%8ld",
           curntn->metals);
  mvprintw(15, COLS / 2 + 1, "%8ld people in mines......%8ld", spread.inmetal,
           spread.metal - curntn->metals);
  standout();
  mvprintw(16, COLS / 2 + 1, "ESTIMATE METAL SUPPLY.........%8ld",
           spread.metal);
  standend();
  ...
  while (done == FALSE) {
    ...
    while ((nvynum < MAXNAVY) && (count < MAXINSCR)) {
      if ((P_NWSHP != 0) || (P_NMSHP != 0) || (P_NGSHP != 0)) {
        ...
        mvprintw(ypos + 2, xpos, "%2hd/%2hd/%2hd", P_NWAR(N_LIGHT),
                 P_NWAR(N_MEDIUM), P_NWAR(N_HEAVY));
        mvprintw(ypos + 3, xpos, "%2hd/%2hd/%2hd", P_NMER(N_LIGHT),
                 P_NMER(N_MEDIUM), P_NMER(N_HEAVY));
        mvprintw(ypos + 4, xpos, "%2hd/%2hd/%2hd", P_NGAL(N_LIGHT),
                 P_NGAL(N_MEDIUM), P_NGAL(N_HEAVY));
        mvprintw(ypos + 5, xpos, "%d", (int) rel_x(P_NXLOC));
        mvprintw(ypos + 6, xpos, "%d", (int) rel_y(P_NYLOC));
        ...
        if (P_NARMY == MAXARM)
          ...
          if (P_ATYPE < MINLEADER)
            ...
            mvprintw(ypos + 9, xpos, "%s %d", *(unittype + (P_ATYPE % UTYPE)),
                     armynum);
        }
    ...
    standout();
    mvprintw(0, (COLS / 2) - 15 - strlen(curntn->name) / 2,
             "NAVY STATS SUMMARY FOR %s", curntn->name);
    ...
    if ((navy = getch()) == ' ')
      ...
      mvaddstr(ypos++, 0,
               "OPTIONS: 1) TRANSFER / MERGE, 2) SPLIT NAVY, 3) DISBAND NAVY");
      clrtoeol();

#ifdef OGOD
      if (isgod == TRUE)
        mvaddstr(ypos++, 0,
                 "GOD OPTIONS:  4) ADJUST SHIPS, 5) LOCATION, 6) CREW 7) MOVE");
      ypos++;
      clrtoeol();
#endif /* OGOD */
      ...
      switch (getch()) {
        case '1':
          ...
          if (curntn->nvy[newnavy].commodity == TRADED) {
            ...
            for (i = N_LIGHT; i <= N_HEAVY; i++) {
              ...
              if (P_NMER(i) + SHIPS(curntn->nvy[newnavy].merchant, i) >
                  N_MASK) {
                i = (-1);
                break;
              }
              if (P_NWAR(i) + SHIPS(curntn->nvy[newnavy].warships, i) >
                  N_MASK) {
                i = (-1);
                break;
              }
            ...
            if (i == (-1)) {
              errormsg(
                  "That would cause you to have too many ships of one type");
              break;
            }
        ...
        case '2':
          ...
          if (P_NWSHP != 0)
            mvaddstr(
                LINES - 2, 0,
                "Do you wish to separate warships from remainder of fleet?");
          refresh();
          ...
          for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
            if (P_NWAR(shipsize) == 0)
              continue;
            mvprintw(ypos, 0, "How Many %s Warships To Split?",
                     fltstr[shipsize]);
            clrtoeol();
            refresh();
            newnavy = get_number();
          ...
          for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
            if (P_NMER(shipsize) == 0)
              continue;
            mvprintw(ypos, 0, "How Many %s Merchants To Split?",
                     fltstr[shipsize]);
            clrtoeol();
            refresh();
            newnavy = get_number();
          ...
          for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
            if (P_NGAL(shipsize) == 0)
              continue;
            mvprintw(ypos, 0, "How Many %s Galleys To Split?",
                     fltstr[shipsize]);
            clrtoeol();
            refresh();
            newnavy = get_number();
        ...
        case '4':
          if (isgod == TRUE) {
            /* ADJUST SHIPS */
            for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
              mvprintw(ypos, 0, "How Many %s Warships? [%d] ", fltstr[shipsize],
                       P_NWAR(shipsize));
              clrtoeol();
              refresh();
              newnavy = get_number();
              if (newnavy < 0 || newnavy > N_MASK)
                continue;
            ...
            for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
              mvprintw(ypos, 0, "How Many %s Galleys? [%d]", fltstr[shipsize],
                       P_NGAL(shipsize));
              clrtoeol();
              refresh();
              newnavy = get_number();
