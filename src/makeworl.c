...
void
makeworld(rflag)
  int             rflag;	/* TRUE if you wish to read in a map from
				 * mapfiles */
{
  ...
  sprintf(newstring, "rm -f %s* %s* %s* %s* %s %s digest 2> /dev/null",
...
{				/* create world */
  ...
  move(14, 0);
  clrtoeol();
  avvalue = (((float) (100 - pwater) / 25.0));	/* Average water tvalue of
						 * sectors */
  for (i = 0; i < MAXX; i++)
    for (j = 0; j < MAXY; j++)
...
populate()
{
  ...
  int             nvynum = 0,
                  ...
                  cnum;
  short           short1,
                  short2;	/* temporary short variables */
  short           class;
