 ...
 * y
 *
 * Revision 1.3  1991/08/07  23:41:54  quixadhal
 * Added ifdefs to shut the nomad/savage captures up.  This is
 * quite annoying to see N pages of news about...
 *
...
#define SEE_CITIES(city_nation, country)                                       \
  ((magic(city_nation, THE_VOID) != TRUE) || (magic(country, NINJA) == TRUE))
...
void prtattr(void) {
  ...
  for (y = (int) curntn->capy - 3; y < (int) curntn->capy + 4; y++) {
    printf("\n");
    for (x = (int) curntn->capx - 3; x < (int) curntn->capx + 4; x++)
      if (ONMAP(x, y)) {
    ...
    for (x = (int) curntn->capx - 3; x < (int) curntn->capx + 4; x++)
      if (ONMAP(x, y)) {
        if (sct[x][y].altitude != WATER) {
          printf("%5d %c%c%c ", attr[x][y], sct[x][y].vegetation,
                 sct[x][y].altitude, sct[x][y].designation);
        } else
          printf("0      ~  ");
      }
    printf("\n");
...
void newdip(int ntn1, int ntn2)
{
  ...
  if ((ntn[ntn1].race == ORC) || (ntn[ntn2].race == ORC)) {
    ...
    if (rand() % 2 < 1)
      ntn[ntn1].dstatus[ntn2] = FRIENDLY;
    else
      ntn[ntn1].dstatus[ntn2] = NEUTRAL;
  } else
    ntn[ntn1].dstatus[ntn2] = NEUTRAL;
...
void monster(void) {
  for (country = 1; country < NTOTAL; country++) {
    ...
    if (curntn->active == NPC_NOMAD)
      do_nomad();
    else if (curntn->active == NPC_PIRATE)
      do_pirate();
    else if (curntn->active == NPC_SAVAGE)
      do_savage();
    else if (curntn->active == NPC_LIZARD)
      do_lizard(); /* update.c */
  }
...
#ifdef MORE_MONST
  {
    ...
    /*
     * neededarmies is an estimete of the desired military strength of
     * savages and nomads. The proportions are based on npc.c routines.
     * yeuch.
     */
    ...
    while (neededtroops > 0 && (nomad_space || savage_space)) {
      ...
      found_free_nomad_army:
        ...
        do {
          x = (rand() % (MAPX - 8)) + 4;
          y = (rand() % (MAPY - 8)) + 4;
        } while (sct[x][y].altitude == PEAK || sct[x][y].altitude == WATER ||
                 (sct[x][y].owner != 0 && sct[x][y].owner != savages &&
                  sct[x][y].people >= 50));
    ...
    if (P_ASOLD > 0) {
      ...
      while (TRUE) {
        ...
        if ((x == P_AXLOC && y == P_AYLOC) ||
            (!ONMAP(x, y) || (!is_habitable(x, y))) ||
            (!land_reachp((int) P_AXLOC, (int) P_AYLOC, x, y, curntn->maxmove,
                          country)))
          continue;
...
void n_redes(int x, int y, int goldthresh, int metalthresh, int citythresh, float hunger)
{
  ...
  if ((sptr->designation == DTOWN) && (hunger < P_EATRATE) &&
  ...
  if ((sptr->designation != DTOWN) && (sptr->designation != DCITY) &&
      (sptr->designation != DCAPITOL)) {
    if ((sptr->tradegood != TG_none) && (tg_ok(sptr->owner, sptr))) {
      if ((metalthresh + goldthresh > 8) ||
          ((sptr->metal < metalthresh) && (sptr->metal != 0)) ||
          ((sptr->jewels < goldthresh) && (sptr->jewels != 0))) {
        sptr->designation = DFARM;
      } else
        sptr->designation = *(tg_stype + sptr->tradegood);

      if ((sptr->metal < metalthresh) && (sptr->metal != 0))
      ...
      sptr->designation = DSTOCKADE;
  }
  if ((sptr->designation == DFARM) && (hunger > P_EATRATE * 1.5) &&
      (tofood(sptr, sptr->owner) <= 6)) {
    if ((rand() % 2 == 0) && (curntn->mine_ability < 30))
      sptr->designation = DBLKSMITH;
    else if ((rand() % 2 == 0) && (sptr->people < 100) &&
             (curntn->spoilrate > 15))
...
void redomil(void) {
    ...
    if (P_ASOLD > 0) {
      ...
      for (x = (int) P_AXLOC - 3; x <= (int) P_AXLOC + 3; x++)
        for (y = (int) P_AYLOC - 3; y <= (int) P_AYLOC + 3; y++)
          if ((ONMAP(x, y)) && (sct[x][y].owner == country))
            ok = 1;
      if (ok == 0) {
        P_AXLOC = curntn->capx;
        P_AYLOC = curntn->capy;
      ...
      if (P_ATYPE == A_MILITIA) {
        ...
        if (((sct[P_AXLOC][P_AYLOC].designation != DTOWN) &&
          ...
          printf("\teliminating %s army %d as %d %d is des:%c alt:%c own:%d\n",
                 unittype[P_ATYPE], armynum, P_AXLOC, P_AYLOC,
                 sct[P_AXLOC][P_AYLOC].designation,
                 sct[P_AXLOC][P_AYLOC].altitude, sct[P_AXLOC][P_AYLOC].owner);
#endif /* DEBUG */

          if (sct[P_AXLOC][P_AYLOC].owner == country)
            sct[P_AXLOC][P_AYLOC].people += P_ASOLD;
          else
            sct[curntn->capx][curntn->capy].people += P_ASOLD;
          P_ASOLD = 0;
        }
      }
      /* set default status */
  ...
  if ((P_ASOLD * 10) < (9 * ideal)) {
    ...
    if (magic(country, WARRIOR) == 1) /* take WARRIOR power into account */
      ...
      curntn->tgold -= diff * *(u_encost + (P_ATYPE % UTYPE));
  ...
  for (armynum = 1; armynum < MAXARM; armynum++)
    if ((P_ASOLD > 0) && (P_ATYPE != A_MILITIA) && (P_ATYPE < MINLEADER) &&
        (P_ASOLD < TAKESECTOR) && (curntn->tgold > 0) &&
        (curntn->metals >=
         (TAKESECTOR + 20 - P_ASOLD) * *(u_enmetal + (P_ATYPE % UTYPE))) &&
      ...
      if (magic(country, WARRIOR) == TRUE) /* WARRIOR power */
        curntn->tgold -=
            ((TAKESECTOR + 20 - P_ASOLD) * *(u_encost + (P_ATYPE % UTYPE))) / 2;
      else
        curntn->tgold -=
            (TAKESECTOR + 20 - P_ASOLD) * *(u_encost + (P_ATYPE % UTYPE));
  ...
  if (curntn->tmil < ((4 * ideal) / 5)) {
    for (armynum = 1; armynum < MAXARM; armynum++)
      if ((done == FALSE) && (P_ASOLD == 0)) {
        ...
        P_ATYPE = defaultunit(country);
        P_ASOLD =
            min((int)(ideal - curntn->tmil),
                (int)(curntn->metals / (*(u_enmetal + (P_ATYPE % UTYPE)))));

        P_ASOLD = min(P_ASOLD, sct[curntn->capx][curntn->capy].people / 2);
        P_ASOLD = min(P_ASOLD,
                      (int)(curntn->tgold / *(u_encost + (P_ATYPE % UTYPE))));
        if (P_ASOLD > 0) {
          ...
          if (magic(country, WARRIOR) == TRUE) /* WARRIOR power */
            ...
            curntn->tgold -= P_ASOLD * *(u_encost + (P_ATYPE % UTYPE));
          sct[P_AXLOC][P_AYLOC].people -= P_ASOLD;
          P_ASTAT = DEFEND;
          P_AMOVE = 0;
        } else
    ...
    for (armynum = 1; done == FALSE && armynum < MAXARM; armynum++) {
      if ((P_ASOLD <= 0) || (P_ATYPE == A_ZOMBIE) || (P_ATYPE == A_MILITIA) ||
          (P_ATYPE >= MINLEADER) || (P_ASTAT == ONBOARD) || (P_ASTAT == TRADED))
        continue;
        ...
        for (i = 1; free == FALSE && i < MAXARM; i++) {
          if (curntn->arm[i].sold == 0) {
            ...
            curntn->arm[i].yloc = P_AYLOC;
                   ...
                   (P_ASOLD < TAKESECTOR || P_ASTAT == MILITIA) &&
                   P_ASTAT != ONBOARD && P_ASTAT != TRADED) {
        free = FALSE;
        for (i = 1; free == FALSE && i < MAXARM; i++) {
          if ((curntn->arm[i].sold > 0) && (curntn->arm[i].stat != ONBOARD) &&
...
#ifdef DEBUG
            printf("\tMerge %ld men from army %d to make %ld troops in %s army "
                   "%d \n",
          ...
          if (free == TRUE) {
            ...
            if (ideal > 0) {
              if (magic(country, WARRIOR) == TRUE) { /* WARRIOR power */
                ...
                curntn->tgold -= (ideal - P_ASOLD) * *(u_encost + P_ATYPE);
              }
            }

#ifdef DEBUG
            printf("\tnow in sector %d,%d\n", x, y);
            printf("\tadding %ld troops to %s army %d (now %ld men - populace "
                   "%ld)\n",
                   ideal - P_ASOLD, unittype[P_ATYPE % UTYPE], armynum, ideal,
                   sct[x][y].people);
#endif /* DEBUG */
...
void getdstatus(void) {
  ...
  if ((curntn->active == GOOD_6FREE) || (curntn->active == ISOLATIONIST) ||
      (curntn->active == NEUTRAL_6FREE) || (curntn->active == EVIL_6FREE))
    svhostile = 5;
  else if ((curntn->active == GOOD_4FREE) ||
           (curntn->active == NEUTRAL_4FREE) || (curntn->active == EVIL_4FREE))
    svhostile = 10;
  else if ((curntn->active == GOOD_2FREE) ||
           (curntn->active == NEUTRAL_2FREE) || (curntn->active == EVIL_2FREE))
    svhostile = 20;
  else if ((curntn->active == GOOD_0FREE) ||
           (curntn->active == NEUTRAL_0FREE) || (curntn->active == EVIL_0FREE))
    svhostile = 35;
  else
    svhostile = 5;
  ...
  for (x = 1; x < NTOTAL; x++)
    if (isntn(ntn[x].active)) {
      ...
      if ((curntn->dstatus[x] == JIHAD) || (curntn->dstatus[x] == UNMET) ||
          (ispc(curntn->active)))
        continue;
      ...
      if ((curntn->dstatus[x] == WAR) && (ntn[x].dstatus[country] < WAR))
        if (rand() % 100 <= friendly)
          curntn->dstatus[x]--;

      if ((curntn->dstatus[x] < WAR) && (curntn->dstatus[x] > ALLIED)) {
      ...
      if (rand() % 100 <= hostile) {
        if ((curntn->dstatus[x] != JIHAD) && (curntn->dstatus[x] != TREATY))
          curntn->dstatus[x]++;
      }
      if ((rand() % 100 <= friendly) && (curntn->dstatus[x] != TREATY) &&
          (curntn->dstatus[x] != JIHAD) && (curntn->dstatus[x] != WAR))
        curntn->dstatus[x]--;
    }
  for (x = 1; x < NTOTAL; x++)
    if (isntn(ntn[x].active)) {
...
static void find_avg_sector(void) {
  ...
  for (nation = 1; nation < NTOTAL; nation++) {
    if (isntn(ntn[nation].active) && !COUNT_ARMIES(nation, country)) {
      /*
       * Count the number of sectors which are occupied by each nation's
       * armies.
       */
      total_sectors = 0;
      for (armynum = 1; armynum < MAXARM; armynum++)
...
void nationrun(void) {
  ...
  for (x = 1; x < NTOTAL; x++)
    if (isntn(ntn[x].active)) {
      /*
       * here is the bit which will occasionally send a randomly generated
       * message from a hostile NPC to a PC (25% chance)
       */
      if ((curntn->dstatus[x] >= HOSTILE) && (ispc(ntn[x].active)))
        if (rand() % 4 == 0) { /* send the message!! */
          if (mailopen(x) != (-1)) {
            fprintf(fm, "Message to %s from %s (%s of year %d)\n\n",
                    ntn[x].name, curntn->name, PMONTH(TURN), YEAR(TURN));
    ...
    if (curntn->tmil == 0)
      ...
      for (x = 0; x < NTOTAL; x++)
        if (isntn(ntn[x].active) && (curntn->dstatus[x] > HOSTILE)) {
          if (100 * (curntn->tmil * (curntn->aplus + 100)) /
                  ((curntn->tmil * (curntn->aplus + 100)) +
                   (ntn[x].tmil * (ntn[x].dplus + 100))) > rand() % 100) {
            /* attacker */
            for (armynum = 1; armynum < MAXARM; armynum++)
  ...
  if (isnpc(curntn->active) && (curntn->active != ISOLATIONIST)) {
    if (isgood(curntn->active)) {
      if (loop <= 1)
        curntn->active = GOOD_0FREE;
      else if (loop >= 6)
        curntn->active = GOOD_6FREE;
      else if (loop >= 4)
        curntn->active = GOOD_4FREE;
      else
        curntn->active = GOOD_2FREE;
      ...
      if (loop <= 1)
        curntn->active = EVIL_0FREE;
      else if (loop >= 6)
        curntn->active = EVIL_6FREE;
      else if (loop >= 4)
        curntn->active = EVIL_4FREE;
      else
        curntn->active = EVIL_2FREE;
    }
  }
  ...
  if ((curntn->tsctrs < 20) || (curntn->score < 20)) {
    ...
    curntn->tax_rate = (int) min((int)(curntn->prestige / 5),
                                 (int)((curntn->popularity + curntn->terror +
                                        3 * curntn->charity) / 10));
  ...
  for (loop = 1; loop < 5; loop++) {
    ...
    for (x = stx; x < endx; x++)
      for (y = sty; y < endy; y++)
        if ((sct[x][y].owner == country) && (is_habitable(x, y)) &&
      ...
      for (y = sty; y < endy; y++)
        if ((sct[x][y].owner == country) &&
            ((sct[x][y].designation == DTOWN) ||
             (sct[x][y].designation == DCITY) ||
             (sct[x][y].designation == DCAPITOL) ||
             (sct[x][y].designation == DFORT)) && (sct[x][y].fortress < 10) &&
            (curntn->tgold > 10000) && (rand() % 5 == 0) &&
  ...
  if (magic(country, VAMPIRE) != TRUE) {
    ...
    if (magic(country, WARLORD) == TRUE)
      ...
      i = 20;
    else if (magic(country, WARRIOR) == TRUE)
      i = 10;
...
void n_trespass(void) {
  ...
  for (x = stx; x < endx; x++)
    for (y = sty; y < endy; y++) {
      if ((sct[x][y].owner != country) && (sct[x][y].owner != 0) &&
...
void n_defend(register short natn)
{
  ...
  if (COUNT_ARMIES(natn, country)) {
    ...
    /*
     * For every of this country's sectors that has a 'natn' army in it, add
     * 1/10 of the average number of soldiers in that 'natn' army.
     */
    for (j = 1; j < MAXARM; j++)
      if (ntn[natn].arm[j].sold > 0) {
        x = ntn[natn].arm[j].xloc;
        y = ntn[natn].arm[j].yloc;
        if (sct[x][y].owner == country) {
          /*
       	   * insure the average soldiers is only added once per sector
       	   */
          repeat = FALSE;
          for (i = 1;(i < j) && !repeat; ++i)
            if ((ntn[natn].arm[i].xloc == x) && (ntn[natn].arm[i].yloc == y))
              repeat = TRUE;
  ...
  for (x = stx; x < endx; x++)
    for (y = sty; y < endy; y++) {
      if (movecost[x][y] == 1)
        attr[x][y] += 50;
      else if (movecost[x][y] <= 3)
        attr[x][y] += 20;
      else if (movecost[x][y] <= 5)
        attr[x][y] += 10;
...
void n_survive(void) {
  ...
  for (nation = 1; nation < NTOTAL; nation++)
    if ((isntn(ntn[nation].active)) && ((ntn[nation].dstatus[country] >= WAR) ||
        ...
        for (armynum = 1; armynum < MAXARM; armynum++)
          if ((ntn[nation].arm[armynum].sold > 0) &&
            ...
            if ((ntn[nation].arm[armynum].xloc == capx) &&
              ...
              attr[ntn[nation].arm[armynum].xloc][ntn[nation].arm[armynum]
                                                      .yloc] +=
                  ntn[nation].arm[armynum].sold;
            }
          }
      } else {
        /*
       	 * The next line might be more accurate to take into account the
       	 * actual number of armies that natn has, but I don't think that VOID
       	 * or VOID should allow that to be known.
       	 */
        for (armynum = 1; armynum < MAXARM; armynum++)
          if (ntn[nation].arm[armynum].sold > 0) {
            x = ntn[nation].arm[armynum].xloc;
            ...
            if ((capx - 2 <= x && x <= capx + 2) &&
              ...
              if (!repeat) {
                if ((ntn[nation].arm[armynum].xloc == capx) &&
                  ...
                  attr[ntn[nation].arm[armynum].xloc][
                      ntn[nation].arm[armynum].yloc] += Avg_soldiers[nation];
                }
              }
            }
          }
