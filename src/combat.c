...
fight()
{
  ...
  int             i,
                  ...
                  k;
  long            asold = 0,
                  dsold = 0;	/* a's and d's total soldiers */
  float           astr = 0,
                  dstr = 0;	/* a's and d's relative strength */
  long            Aloss,
                  Dloss;	/* a's and d's total losses */
  int             PAloss,
                  PDloss;	/* percent a and d loss */
  long            loss;
  int             abonus = 0,
                  dbonus = 0;	/* bonus aggregate */
...
printf("Entering fight()\n");
  ...
  for (j = 0; j < count; j++)
	...
	side[j] = DFND;
      else if ((ntn[owner[j]].dstatus[anation] == TREATY) && (ntn[owner[j]].dstatus[dnation] > HOSTILE))
	side[j] = ATKR;
      else if ((ntn[owner[j]].dstatus[anation] == ALLIED) && (ntn[owner[j]].dstatus[dnation] > HOSTILE))
	side[j] = ATKR;
    }
...
{				/* finds retreat location */
  ...
  for (x = xsctr - 1; x <= xsctr + 1; x++)
    for (y = ysctr - 1; y <= ysctr + 1; y++)
      if (ONMAP(x, y)) {
	if (tofood(&sct[x][y],
		   sct[x][y].owner == country ? country : 0) == 0)
	  continue;
...
void
retreat(unitnum)
  int             unitnum;	/* if -1 then normal, else retreat only unit
				 * ismerc */
{
  int             cnum;
...
{
  int             acrew = 0,
                  dcrew = 0;	/* a's and d's crew and soldier strength */
  int             ahold = 0,
                  dhold = 0;	/* a's and d's warship strength */
  int             awsunk = 0,
                  dwsunk = 0;	/* a's and d's warship losses for the round */
  int             agsunk = 0,
                  dgsunk = 0;	/* a's and d's galley losses for the round */
  int             amsunk = 0,
                  dmsunk = 0;	/* a's and d's merchent losses for the round */
  int             awcapt = 0,
                  dwcapt = 0;	/* a's and d's warship captures for the round */
  int             agcapt = 0,
                  dgcapt = 0;	/* a's and d's galley captures for the round */
  int             amcapt = 0,
                  dmcapt = 0;	/* a's and d's merchant captures for the
				 * round */
  int             akcrew = 0,
                  dkcrew = 0;	/* a's and d's crew losses for the round */
  char            wnum[MGKNUM],
                  gnum[MGKNUM],
                  mnum[MGKNUM];
  register int    done,
