...
extern short hilmode;
    /* highlight modes: 0=owned sectors, 1=
				 * armies, 2=none */
...
void mymove() {
  ...
  if (armynum >= MAXARM) { /* navy */
    ...
    if (P_ASTAT >= NUMSTATUS) {
      mvprintw(LINES - 4, 10, "Member of Army Group %d!! Continue? ",
               P_ASTAT - NUMSTATUS);
      refresh();
      if (getch() == 'y') {
        P_ASTAT = ATTACK;
  ...
  while (done == FALSE) {
    ...
    switch (getch()) {
      ...
      case '': /* redraw map */
        ...
        if (hilmode == 3) {
          ...
          for (i = 0; i < MAXNAVY; i++)
            if (curntn->nvy[i].merchant + curntn->nvy[i].warships +
                    curntn->nvy[i].galleys != 0) {
              standout();
      ...
      case ' ':
        valid = FALSE;
        if (mveleft > 0 && armornvy == ARMY &&
            sct[XREAL][YREAL].altitude == WATER && P_ASTAT == FLIGHT)
    ...
    if (valid == TRUE) {
      if (armornvy == ARMY) {
        if (P_ASTAT == FLIGHT) {
          ...
          if ((P_ASTAT != SCOUT) && (P_ATYPE != A_NINJA) &&
              (P_ATYPE < MINLEADER || P_ATYPE >= MINMONSTER ||
               P_ASTAT == GENERAL) && (sct[XREAL][YREAL].owner != 0) &&
            ...
            errormsg(
                "You May Not Enter Non-Allied Land Without Declaring War.");
            valid = FALSE;
            xcurs = oldxcurs;
            ycurs = oldycurs;
          } else if ((sct[XREAL][YREAL].owner != country) &&
                     (sct[XREAL][YREAL].owner != 0) && (P_ASTAT != SCOUT) &&
        ...
        if (abs(movecost[XREAL][YREAL]) > mveleft) {
          ...
          if (valid == FALSE) {
            ...
            xcurs = oldxcurs;
            ycurs = oldycurs;
          } else if (((sct[XREAL][YREAL].designation == DTOWN) ||
            ...
            if (sct[XREAL][YREAL].owner == country)
              mvaddstr(LINES - 3, 0, "Do you wish to land?");
            else
              mvaddstr(LINES - 3, 0, "Do you wish to invade?");
            standend();
            clrtoeol();
            refresh();
            if (getch() == 'y') {
              ...
              refresh();
                   ...
                   (P_NGAL(N_LIGHT) != 0 || P_NWAR(N_LIGHT) != 0 ||
                    P_NMER(N_LIGHT) != 0)) {
    ...
    if ((valid == TRUE) && (done == FALSE)) {
      ...
      /* if designation of sct[XREAL][YREAL] == fortified and status of owner
       * != ALLIED/TREATIED/FRIENDLY... total += N men
       */
      if (ntn[SOWN].dstatus[country] > FRIENDLY) {
        if (sct[XREAL][YREAL].designation == DSTOCKADE)
          total += 250;
        if (sct[XREAL][YREAL].designation == DFORT)
          total += 500;
      }
      move(LINES - 3, 0);
      clrtoeol();
      ...
      if ((armornvy == ARMY) && (P_ASTAT != SCOUT) && (P_ATYPE != A_NINJA) &&
          ((P_ATYPE < MINLEADER || P_ASTAT == GENERAL ||
            P_ATYPE >= MINMONSTER)) && (P_ASTAT != FLIGHT)) {
        if (groupmen + othermen < total) {
          ...
          P_AMOVE -= total * curntn->maxmove * *(unitmove + (P_ATYPE % UTYPE)) /
                     (10 * (groupmen + othermen));
          if (P_AMOVE > 150)
            P_AMOVE = 0;
          AADJMOV;
          if (P_AMOVE == 0)
            ...
            errormsg("Zone Of Control - Reducing Movement");
        }
        if (P_AMOVE == 0)
          done = TRUE;
    ...
    if (done == FALSE) {
      ...
      if (armornvy == ARMY) {
        ...
        if (P_ASTAT == FLIGHT) {
          ...
          printw(" move cost is %d", movecost[XREAL][YREAL]);
      } else
        mvprintw(LINES - 2, 0, "MOVESCREEN: move left: %d  move cost is %d",
                 P_NMOVE, abs(movecost[XREAL][YREAL]));
      clrtoeol();
      standend();
      mvaddstr(LINES - 1, 0, "HIT SPACE IF DONE");
  ...
  if (armornvy == ARMY) {
    ...
    if (P_ASTAT == FLIGHT) {
      ...
      if (sct[XREAL][YREAL].altitude == WATER) {
        ...
        if (P_ATYPE == A_MERCENARY) {
          mvprintw(LINES - 2, 0, "Paying mercenary families %ld talons",
    ...
    if (((P_ATYPE < MINLEADER) || (P_ASTAT == GENERAL)) && (P_ASOLD > 0)) {
      /* calc enemy soldiers */
      total = 0;
      for (Tnation = 0; Tnation < NTOTAL; Tnation++)
        if (Tnation != country)
      ...
      if (TAKEUNOWN(groupmen, SOWN)) {
        ...
        AADJMOV;
                 ...
                 ((sct[P_AXLOC][P_AYLOC].designation == DTOWN) ||
                  (sct[P_AXLOC][P_AYLOC].designation == DFORT) ||
                  (sct[P_AXLOC][P_AYLOC].designation == DCAPITOL) ||
                  (sct[P_AXLOC][P_AYLOC].designation == DCITY))) {
        mvaddstr(LINES - 2, 0, "Entering Fortified sector");
        clrtoeol();
        refresh();
        sleep(2);
      } else if ((SOWN != country) &&
                 (groupmen >= TAKEPOPUL(sct[XREAL][YREAL].people)) &&
                 ((occ[XREAL][YREAL] == 0) || (occ[XREAL][YREAL] == country) ||
        ...
        P_AMOVE = 0;
        AADJMOV;
      } else if ((sct[XREAL][YREAL].owner != country) && (total > 0) &&
                 (groupmen < TAKEPOPUL(sct[XREAL][YREAL].people)) &&
                 (P_ASTAT != SCOUT) &&
                 ((P_ATYPE < MINLEADER) || (P_ASTAT == GENERAL))) {
        clear_bottom(0);
        mvprintw(
            LINES - 3, 0,
            "army has too few men (%d) to take sector (need %d) - hit any key",
            groupmen, TAKEPOPUL(sct[XREAL][YREAL].people));
        refresh();
        getch();
      }
