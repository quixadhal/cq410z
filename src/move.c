...
extern short    xcurs,
                ...
                xoffset,
                yoffset;
extern short    hilmode;	/* highlight modes: 0=owned sectors, 1=
				 * armies, 2=none */
...
mymove()
{
  ...
  while (done == FALSE) {
	...
	for (i = 0; i < MAXNAVY; i++)
	  if (curntn->nvy[i].merchant + curntn->nvy[i].warships + curntn->nvy[i].galleys != 0) {
	    standout();
 ...
 */
  if(ntn[SOWN].dstatus[country] > FRIENDLY) {
    if(sct[XREAL][YREAL].designation == DSTOCKADE) total+= 250;
    if(sct[XREAL][YREAL].designation == DFORT) total+= 500;
  }
      move(LINES - 3, 0);
      clrtoeol();
	...
	mvprintw(LINES - 2, 0, "MOVESCREEN: move left: %d  move cost is %d", P_NMOVE, abs(movecost[XREAL][YREAL]));
      clrtoeol();
      standend();
      mvaddstr(LINES - 1, 0, "HIT SPACE IF DONE");
      clrtoeol();
  ...
  if (armornvy == ARMY) {
	...
	clear_bottom(0);
	mvprintw(LINES - 3, 0, "army has too few men (%d) to take sector (need %d) - hit any key", groupmen, TAKEPOPUL(sct[XREAL][YREAL].people));
	refresh();
	getch();
      }
      clrtoeol();
