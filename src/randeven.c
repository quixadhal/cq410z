 ...
 * $State: Exp $
 * $Log: randeven.c,v $
 * Revision 2.2  1991/10/14  02:33:33  cclub
 * Fixed volcano bug... I hope...
 *
 * Revision 2.1  1991/09/20  16:54:46  cclub
 * y
 *
 * Revision 1.4  1991/08/09  04:11:23  quixadhal
 * Added relative coords to mail routines.
 *
...
char *names[] = { /* must end in single character name */
                  "aghast", "agony", "anger", "anguish", "misery", "suffer",
                  "torture", "pain", "death", "gasp", "cry", "weep", "scream",
                  "demise", "grim", "gore", "fester", "blood", "bone", "howl",
                  "terror", "shock", "shriek", "screech", "yowl", "cadaver",
                  "corpse", "slice", "carve", "rip", "burn", "crumble", "sting",
                  "screwed", "fucked", "lost", "gone", "filth", "pus", "scab",
                  "whipped", "beaten", "blasted", "dust", "rout", "smear",
                  "trounce", "flambe", "smother", "sicken", "choke", "strangle",
                  "garrotte", "nuked", "squish", "plaster", "argh", "X" };
...
int findnew(void) {
  ...
  for (nationis = NTOTAL - 1; nationis >= 1; nationis--)
    if (ntn[nationis].active == INACTIVE)
      newntn = nationis;
  if (newntn == 0)
    return (0);
...
char getnewmark(void) {
  ...
  while (TRUE) {
    tmpchr++;
    if (markok(tmpchr, FALSE))
      break;
    if (tmpchr == 'Z')
      tmpchr = 'a' - 1; /* then small letters */
    if (tmpchr == 'z')
      break; /* otherwise it will loop forever */
  }
  return (tmpchr);
}
...
int disolve(int target, int percent, int ispsnt /* true/false */)
{
...
#else
  sprintf(eventstr, "new nation %s created at %d,%d", ntn[new].name, realx,
          realy);
#endif /* HIDELOC */
  ...
  if (marknearok(toupper(ntn[new].name[0]), FALSE))
  ...
  if (!ispsnt)
    for (armynum = narmynum; armynum < MAXARM; armynum++) {
      ...
      if ((ntn[target].arm[armynum].sold > 0) &&
          (sct[ntn[target].arm[armynum].xloc][ntn[target].arm[armynum].yloc]
               .owner == new)) {
        ntn[new].arm[narmynum].sold = ntn[target].arm[armynum].sold;
        ntn[new].arm[narmynum].unittyp = ntn[target].arm[armynum].unittyp;
        ntn[new].arm[narmynum].xloc = ntn[target].arm[armynum].xloc;
        ntn[new].arm[narmynum].yloc = ntn[target].arm[armynum].yloc;
        ntn[new].arm[narmynum].stat = ntn[target].arm[armynum].stat;
        ntn[new].arm[narmynum].smove = ntn[target].arm[armynum].smove;
...
void randomevent(void) {
  ...
  for (country = 0; country < NTOTAL; country++) {
    curntn = &ntn[country];
    if ((!isntn(curntn->active)) || (curntn->score <= 20L) ||
        (curntn->tsctrs <= 20))
      continue;
    ...
    x = 10 * curntn->tax_rate - curntn->popularity - curntn->terror -
        3 * curntn->charity;

#ifdef DEBUG
    printf(
        "TEMP: %s chance of peasant revolt is %d (tax=%d pop=%d terror=%d)\n",
        curntn->name, x, curntn->tax_rate, curntn->popularity, curntn->terror);
#endif /* DEBUG */
    ...
    if ((rand() % 100) < x) {
      if (rand() % 100 < PREVOLT) {
        ...
        if (holdval != 0)
          wdisaster(country, ntn[holdval].capx, ntn[holdval].capy, 0,
                    "peasant revolt");
        else
          printf("revolt in %s fails because:\n\t%s\n", curntn->name, eventstr);
      }
    }
    ...
    printf("TEMP: %s chance of revolt is %d (tax=%d prest=%d)\n", curntn->name,
           x, curntn->tax_rate, curntn->prestige);
#endif /* DEBUG */
    ...
    if ((rand() % 100) * WORLDSCORE < RANEVENT * WORLDNTN * curntn->score) {
      ...
      switch (event) {
        ...
        case 12:
          /* tornado */
          sptr = rand_sector();
          if (!(ISCITY(sptr->designation)) && (sptr->designation != DRUIN))
            sptr->designation = DNODESIG;
          else {
            if (sptr->fortress < 2)
              sptr->fortress = 0;
        ...
        case 18:
          ...
          for (i = 0; i < MAPX; i++)
            for (j = 0; j < MAPY; j++)
              if ((sct[i][j].owner == country) && (sct[i][j].jewels == 0) &&
                  (is_habitable(i, j))) {
                holdval++;
              }
          if (holdval > 0)
          ...
          for (xpos = 0; count && (xpos < MAPX); xpos++)
            for (ypos = 0; count && (ypos < MAPY); ypos++)
              if ((sct[xpos][ypos].owner == country) &&
                  (sct[xpos][ypos].jewels == 0) && (is_habitable(xpos, ypos))) {
                if (count-- <= 0) {
        ...
        case 25:
          ...
          for (holdval = 1; holdval < NTOTAL; holdval++)
            if (ntn[holdval].active == NPC_NOMAD)
              break;
        ...
        case 26:
          ...
          for (xpos = 0; count && (xpos < MAPX); xpos++)
            for (ypos = 0; count && (ypos < MAPY); ypos++)
              if ((sct[xpos][ypos].owner == country) && (done == FALSE) &&
                  (sct[xpos][ypos].designation == DTOWN)) {
                count--;
                if (count <= 0) {
                  count = 0;
          ...
          if ((newpower = getmagic(M_CIV)) != 0L) {
            for (i = S_CIV; i < S_CIV + E_CIV; i++)
              if (powers[i] == newpower) {
                sprintf(eventstr, "nation %s gets civilian power %s",
                        curntn->name, pwrname[i]);
              }
            exenewmgk(newpower);
          } else
            done = FALSE;
        ...
        case 31: /* new magic item + RANDOM POWER */
          ...
          if ((newpower = getmagic(M_MIL)) != 0) {
            for (i = S_MIL; i < S_MIL + E_MIL; i++)
              if (powers[i] == newpower)
                sprintf(eventstr, "nation %s gets military power %s",
                        curntn->name, pwrname[i]);
            exenewmgk(newpower);
          } else
            done = FALSE;
        ...
        case 34: /* new ores discovered */
          ...
          for (i = 0; i < MAPX; i++)
            for (j = 0; j < MAPY; j++)
              if ((sct[i][j].owner == country) && (is_habitable(i, j)) &&
                  (sct[i][j].metal == 0))
                holdval++;
          ...
          for (xpos = 0; count && (xpos < MAPX); xpos++)
            for (ypos = 0; count && (ypos < MAPY); ypos++)
              if ((sct[xpos][ypos].owner == country) && (done == FALSE) &&
                  (is_habitable(xpos, ypos)) && (sct[xpos][ypos].metal == 0)) {
                if (count-- <= 0) {
        ...
        case 35:
          ...
          for (newnation = 0; newnation < NTOTAL; newnation++)
            if ((country != newnation) && (isntn(ntn[newnation].active)) &&
        ...
        case 36:
          ...
          if (done == TRUE) {
            if (magic(country, newpower) == TRUE) {
              ...
              removemgk(newpower);
              sprintf(eventstr, "nation %s loses %s power", curntn->name,
                      pwrname[i]);
            } else {
              done = FALSE;
        ...
        case 38:
          ...
          break;
        case 39: /* ninja attack paralyzes half your armys
				 * P_AMOVE=0 */
      ...
      if (done)
        wdisaster(country, xpos, ypos, percent, randevents[event]);
    }
  }
}
...
void peasant_revolt(int *newnation /* return nation id */)
{
  ...
  if ((*newnation = disolve(10, country, TRUE)) == 0)
    return;
  ...
  for (i = 0; i < MAPX; i++)
    for (j = 0; j < MAPY; j++) {
      if ((sct[i][j].owner == country) && (rand() % 2 == 0) &&
          (sct[i][j].designation != DCAPITOL) && (sct[i][j].people > 0) &&
...
int other_revolt(int *new /* return reason and new nation number */)
{
  ...
  switch (reason) {
    case 0: /* general */
      if ((*new = disolve(10, country, FALSE)) != 0)
        return (reason);
      break;
    case 1: /* cult */
      if ((*new = disolve(10, country, FALSE)) != 0)
    ...
    case 3: /* evil wizard sets up */
      ...
      break;
    case 4: /* tax revolt */
      if ((*new = disolve(20, country, FALSE)) != 0)
        return (reason);
      break;
    case 5: /* open rebellion */
      if ((*new = disolve(30, country, FALSE)) != 0)
        return (reason);
      break;
...
void blowup(register int i, register int j)
{
  ...
  fprintf(fnews,
          "1. \tevent in sector %d, %d->volcanic eruption causes devastation\n",
          relx, rely);
