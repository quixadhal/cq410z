...
void get_display_mode(short *dmode, short *hmode, short *odmode, short *ohmode)
{
  ...
  mvaddstr(LINES - 4, 0, "viewing options:  (d)esignation, (r)ace, (M)ove "
                         "cost, (p)eople, (D)efense");
  clrtoeol();
  mvaddstr(LINES - 3, 0, "   (f)ood, (c)ontour, (v)egetation, (m)etal, "
                         "(n)ation mark, (j)ewels, (i)tems");
  clrtoeol();
  mvaddstr(LINES - 2, 0, "highlight option: (o)wned, (a)rmy, (y)our Army, move "
                         "(l)eft, (s)pecial,(x)=none");
  clrtoeol();
  if (odmode != NULL) {
    mvaddstr(LINES - 1, 0,
             "Toggle second display: '/'; set display: '1' or '2'");
  } else {
  ...
  clrtoeol();
  standout();
  mvaddstr(LINES - 1, COLS - 25,
           odmode != NULL ? "what display?:" : "display?:");
  standend();
  refresh();
  redraw = PART;
  switch (getch()) {
...
char get_display_for(int x, int y, short dmode)
{
  ...
  if ((magic(sct[x + xoffset][y + yoffset].owner, THE_VOID) == TRUE) &&
      ...
      (magic(country, NINJA) != TRUE) && (country != 0)) {
    ch = '?';
  } else {
    switch (dmode) {
      case DI_FOOD: /* food */
      ...
      case DI_DESI: /* designation */
        if (sct[x + xoffset][y + yoffset].owner == 0) {
          ...
          ch = ntn[sct[x + xoffset][y + yoffset].owner].mark;
        break;
      ...
      case DI_RACE: /* race */
        if (sct[x + xoffset][y + yoffset].owner == 0)
          ...
          ch = ntn[sct[x + xoffset][y + yoffset].owner].race;
        break;
      case DI_MOVE: /* movement cost map */
        if (movecost[x + xoffset][y + yoffset] >= 0) {
          if (movecost[x + xoffset][y + yoffset] >= 10)
            ...
            ch = movecost[x + xoffset][y + yoffset] + '0';
      ...
      case DI_DEFE: /* Defence */
        if (sct[x + xoffset][y + yoffset].altitude == WATER)
          ...
          /*
         	 * Racial combat bonus due to terrain (the faster you move the
         	 * better)
         	 */
          armbonus = 0;
      ...
      case DI_ITEMS: /* designations needed for tradegoods */
        if (sct[x + xoffset][y + yoffset].altitude == WATER)
          ...
          ch = 'X';
                 ...
                 (*(tg_stype + sct[x + xoffset][y + yoffset].tradegood) !=
  ...
  switch (hmode) {
    case HI_MOVE: /* your armies w/ move left */
      for (armynum = 0; armynum < MAXARM; armynum++)
        if ((P_ASOLD != 0) && (P_AMOVE != 0) && (P_AXLOC == (x + xoffset)) &&
            (P_AYLOC == (y + yoffset)))
          break;
      if (armynum < MAXARM)
        standout();
    ...
    case HI_YARM: /* your armies */
      for (armynum = 0; armynum < MAXARM; armynum++)
        if ((P_ASOLD != 0) && (P_AXLOC == x + xoffset) &&
            (P_AYLOC == y + yoffset))
          break;
      if (armynum < MAXARM)
        standout();
    ...
    case HI_GOOD: /* trade goods */
      if (tg_ok(country, &sct[x + xoffset][y + yoffset]) &&
          ...
          (magic(sct[x + xoffset][y + yoffset].owner, THE_VOID) != TRUE ||
           country == 0 || country == sct[x + xoffset][y + yoffset].owner ||
           magic(country, NINJA) == TRUE) &&
          (sct[x + xoffset][y + yoffset].altitude != WATER))
        standout();
      break;
...
void whatcansee(void) {
  ...
  for (nvynum = 0; nvynum < MAXNAVY; nvynum++)
    if ((P_NMSHP != 0) || (P_NWSHP != 0) || (P_NGSHP != 0))
      for (i = (int) P_NXLOC - xoffset - NAVYSEE;
           i != 1 + (int) P_NXLOC - xoffset + NAVYSEE; i++)
        for (j = (int) P_NYLOC - yoffset - NAVYSEE;
             j != 1 + (int) P_NYLOC - yoffset + NAVYSEE; j++)
  ...
  for (armynum = 0; armynum < MAXARM; armynum++)
    if (P_ASOLD > 0)
      for (i = (int) P_AXLOC - xoffset - ARMYSEE;
           i != 1 + (int) P_AXLOC - xoffset + ARMYSEE; i++)
        for (j = (int) P_AYLOC - yoffset - ARMYSEE;
             j != 1 + (int) P_AYLOC - yoffset + ARMYSEE; j++)
          if (ONMAP(i + xoffset, j + yoffset) && i >= 0 && j >= 0 &&
              i < SCREEN_X_SIZE && j < SCREEN_Y_SIZE)
            HAS_SEEN(i, j) = TRUE;
