 ...
 */
    if(ispc(ntn[x].active)) if(ntn[x].score < s4) {
      n4= x;
      s4= ntn[x].score;
    }
    ...
    if(isntn(ntn[x].active))
      if(ntn[x].score > s3)
        if(ntn[x].score > s2)
          if(ntn[x].score > s1) {
            n3= n2; n2= n1; n1= x;
            s3= s2; s2= s1; s1= ntn[x].score;
          } else {
          n3= n2; n2= x;
          s3= s2; s2= ntn[x].score;
        } else {
        n3= x;
        s3= ntn[x].score;
      }
...
/* cheat in favor of npcs as the create army routines assume 75 man armies */
	if (P_ATYPE < MINLEADER) {
	  if ((ispc(curntn->active) && (TAKEUNOWN(P_ASOLD, sptr->owner) ||
...
fprintf(stderr, "Blabbing about it in the news file...\n");
	...
	if(country) {
		...
		if(relx > (MAPX/2)) relx -= MAPX;
		if(rely > (MAPY/2)) rely -= MAPY;
		if(relx <= -(MAPX/2)) relx += MAPX;
		if(rely <= -(MAPY/2)) rely += MAPY;
	}
...
updsectors()
{
  ...
  for (country = 1; country < NTOTAL; country++) {
	...
	curntn->popularity = max(0, (curntn->popularity - 2 * curntn->inflation));
      } else
...
updmil()
{
  ...
  int             AX,
                  AY,
                  AT;		/* armies x,y locations, type : for speed */
  int             armynum,
                  nvynum,
                  flag,
                  dfltunit;
 ...
 */
	...
	switch(SEASON(TURN+1)) {
	  case SUMMER:  curntn->arm[armynum].smove +=
			  (curntn->arm[armynum].smove / 8);
			break;
          case FALL:    curntn->arm[armynum].smove +=
	                  (curntn->arm[armynum].smove / 2);
			break;
	  case WINTER:  curntn->arm[armynum].smove /= 2;
			break;
          case SPRING:
			break;
        }
...
updleader()
{
  ...
  for (nation = 0; nation < NTOTAL; nation++) {
	...
	switch(SEASON(TURN+1)) {
	  case SUMMER:  P_AMOVE += (P_AMOVE / 8);
			break;
          case FALL:  P_AMOVE += (P_AMOVE / 2);
			break;
	  case WINTER:    P_AMOVE /= 2;
			break;
          case SPRING:
			break;
        }
