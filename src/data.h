...
/* add a routine which acts like markok but ignores duplicates... */

#define markok(m,p) oldmarkok(m,p,FALSE)
#define marknearok(m,p) oldmarkok(m,p,TRUE)
...
#define	PASSLTH		12	/* the number of characters in the passwd */
#define	NAMELTH		12	/* the number of characters in the name */
#define	LEADERLTH	12	/* the number of characters in the leader */
...
#define MINJIHADCOST	200000L
#define	BREAKJIHAD max(MINJIHADCOST,(min(curntn->tgold,ntn[nation].tgold)/4))

/*army status*/
#define	MARCH		1	/* March	 */
#define	SCOUT		2	/* Scouting--will not engage enemy if
				 * possible */
...
#define	NUMSTATUS	17	/* number of possible stats	 */
 /*
  * Army groups are implemented in the army status vbl.  if >= NUMSTATUS, you
  * belong to army group x-NUMSTATUS - army groups always of status attack
  * and may not be magicked
  */

/*seasonal definitions	*/
#define	TURN		world.turn	/* game turn	 */
#define	SEASON(x)	(((x)%12)/3)
#define	PSEASON(x)	seasonstr[(((x)%12)/3)]
#define MONTH(x)	((x)%12)
#define PMONTH(x)	monthstr[((x)%12)]
...
struct s_world {
  ...
  short		  karma;	/* overall world alignment */
  char		  pwd2[PASSLTH + 1];
};
...
struct s_sector {
  ...
  long            foodstore;    /* food stored in sector */
  long            jewelstore;
  long            metalstore;
...
struct s_nation {		/* player nation stats	 */
  ...
  int		  owner;
  /* char		  owner[LEADERLTH + 1]; owner name (uid) */
  ...
  unsigned short   capx;		/* Capitol x coordinate		 */
  unsigned short   capy;		/* Capitol y coordinate		 */
  unsigned short   rlx;		/* relative x coordinate		 */
  unsigned short   rly;		/* relative y coordinate		 */
  unsigned char   active;	/* nation type and strategy	 */
  unsigned char   strategy;	/* npc playing strategy */
...
#define	CHGMGK { fprintf(fexe,"L_MAGIC\t%d\t%hd\t%ld\t%ld\t0\t%s\n",CHG_MGK,country,ntn[country].powers,x,"null"); fflush(fexe); }
...
#define	ECHGLDR fprintf(fexe,"E_CLDR\t%d\t%hd\t0\t0\t0\t%s\n",XECLDR,country,ntn[country].leader)
#define	ECHGMARK fprintf(fexe,"E_CMARK\t%d\t%hd\t0\t0\t0\t%c\n",XECMRK,country,ntn[country].mark)
...
#define class_(NATION,CLASS) (ntn[NATION].class == CLASS)
...
extern struct sprd_sht {
  long            food,
                  gold,
                  jewels,
                  metal;	/* total @ end of turn	 */
  long            revfood,
                  revjewels,
                  revmetal,
                  revcap,
                  revcity,
                  revothr;	/* revenue in turn	 */
  long            ingold,
                  inmetal,
                  infarm,
                  incity,
...
extern char    *des,
               ...
               *pwrname[];
extern long     powers[];
extern char    *tg_value,
               *tg_name[],
               *tg_stype;	/* trade goods	 */
...
#define	STARTYEAR	-10000	/* year that game treats as 0.  In this	case
