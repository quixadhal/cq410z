...
trade()
{
  ...
  while (done == FALSE) {
	...
	if (type1[holdint] == TDLAND) {
	  holdlong = (long) tofood(&sct[abs_x((int) lvar1[holdint])][abs_y(extra[holdint])], 0);
	...
	switch (type2[holdint]) {
	 ...
	 case TDLAND:
	  ...
	  if (checkland(BUY, (int) (holdlong), (int) (holdlong2)) == NODEAL) {
	    buysell = NODEAL;
	  } else if (tofood(&sct[abs_x((int) holdlong)][abs_y((int) holdlong2)], natn[holdint]) < lvar2[holdint]) {
	    tradeerr("You underbid the minimum");
	    buysell = NODEAL;
	  }
	  break;
	...
	if (itemnum >= MAXITM) {
	  ...
	  if (itemnum == 0)
	    ...
	    mvaddstr(count++, 0, "Market Congested.  (B)uy or any key to continue");
	  standend();
	  refresh();
	  break;
	}
  ...
  switch (type) {
   ...
   case TDLAND:
    if (cntry2 == sct[abs_x((int) longval)][abs_y(extint)].owner)
      returnval = (long) tofood(&sct[abs_x((int) longval)][abs_y(extint)], cntry1);
    break;
   case TDARMY:
...
uptrade()
{
  ...
  for (count = 0; count < itemnum; count++) {
	...
	fprintf(tfile, "%d %d %d %d %ld %ld %d\n", deal[count], natn[count], type1[count], type2[count], lvar1[count], lvar2[count], extra[count]);
      ...
      if (longval1 != (-1))
	longval2 = tradeit(whobuy[count], natn[count], type2[count], buy1[count], (int) buy2[count]);
      if (longval1 == (-1)) {
	/* abort due to seller */
