...
int  owneruid;
{
  ...
  for(i= 0; i< 7; i++) if(!strcmp(day_names[i], time_check_value)) {
    ok= 1;
    break;
  }
  if(!ok) return 0;
  ...
  if(!dirarea) sprintf(buffer, "times");
  else sprintf(buffer, "%s/times", dirarea);
  if(!(fp= fopen(buffer, "r"))) return 0;
  for(i= 0; i< 7; i++)
    if(feof(fp)) {
      if(fp) fclose(fp);
      return 0;
    } else fscanf(fp, "%d%d%d", &time_values[i][0], &time_values[i][1],
      &time_values[i][2]);
  if(hour== time_values[day][2]) return 0;
  if(!dirarea) sprintf(buffer, "members");
  else sprintf(buffer, "%s/members", dirarea);
...
to sit\nand watch this drivel scroll.  It's only $5 bucks you know!  Then \
you\nwouldn't have to look at this stupid message every time you play...\n\n",
(getpwuid(getuid()))->pw_name);
  fflush(stdout);
  sleep(20);
  if(time_values[day][0] < 0) return 1;
  if(time_values[day][0] < time_values[day][1]) {
    if(hour < time_values[day][0]) return 0;
    if(hour >= time_values[day][1]) return 0;
  } else
